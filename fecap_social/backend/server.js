...
// cria a tabela usuarios com a coluna RA
db.run(`
...
`, (err) => {
  if (err) {
    ...
    console.log('Tabela usuarios criada (se não existia).');
  }
});
...
app.post('/cadastro', (req, res) => {
  const { usuario, senha, RA } = req.body;

  if (!usuario || !senha) {
    return res.status(400).send('Usuário e senha são obrigatórios.');
  }

  const sql = 'INSERT INTO usuarios (usuario, senha, RA) VALUES (?, ?, ?)';
  db.run(sql, [usuario, senha, RA || null], function (err) {
    if (err) {
